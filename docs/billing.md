...
- Sotto's account branding has the current black-and-white icon/logo and black brand/accent colors, verified after saving in receipt and portal previews. Its activation form has sotto.email, the Sotto public name, software-as-a-service category, SOTTO statement descriptor and the authorized included Radar Lite option. Final account activation remains with the owner.
- The earlier dedicated test portal permits Solo/Duo changes, fixed quantity 1, prorated invoices, payment updates and period-end cancellation. Sotto still needs its own live products, portal, runtime key, signed webhook and account-specific validation; these resources cannot be copied across Stripe accounts by reusing their IDs.
- A commercial Vercel upgrade quote is prepared, pending the founder's confirmation. MCP access to the new Sotto account also requires the owner's account grant; the existing connection still covers Perception only.
