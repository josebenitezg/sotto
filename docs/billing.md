...
As of September 10, 2026, the test product, monthly price, demonstration Payment Link and customer portal are configured. A dedicated restricted test runtime credential and the Google OAuth client credentials have been saved as production secrets and deployed. The Google app has its four required scopes and two pilot test users configured. The Stripe webhook remains disabled, and Checkout and public signup remain disabled until runtime permissions and the complete Google-to-Checkout pilot are verified. Saving credentials and completing a deployment do not validate those flows.
