...
Checkout explicitly sets `managed_payments.enabled=false`. The Sotto account defaulted to Managed Payments, which rejects the custom allowance disclosure. Standard Checkout preserves Sotto as the seller and the configured subscription terms. This was verified against the live API.
...
Full access bypasses billing limits, not Google consent, mailbox isolation, the global write switch or a configured mailbox-write allowlist. The founder authorized the public rollout on September 11: the pilot write restriction was removed, while consent, workspace ownership and processing entitlements remain enforced. The same provider capacity and service limits apply.
...
- Matching Sotto live credentials, prices, portal and signing secret are deployed. Checkout, public signup and installation-wide mailbox writes are enabled after explicit founder authorization. Processing requires an active trial, paid subscription or configured Full access; existing internal accounts retain their access. No external schedule was installed.
- Live deployed Solo/Duo Checkout and customer portal requests passed using one isolated synthetic workspace. Browser review confirmed three days free, then $5/$9 monthly, a required card and the 50/100-email trial disclosure. Retries reused Checkout, and switching plans expired the previous session. No card was entered, subscription created or payment charged.
- A locally signed webhook probe reached the production endpoint, reconciled the synthetic customer against the live Stripe API and correctly retained no processing access despite a forged active status in the fixture. Invalid signatures were rejected and duplicate delivery stored once. This verifies the deployed receiver, not a real Stripe-originated payment or renewal. The earlier sandbox lifecycle validation remains separate.
- The original launch audit found one internal workspace, two connected Composio mailboxes in automatic mode and no existing Stripe customers or billable workspaces. Both managed triggers were verified active with `interval:15`; their V3 webhook points to the deployed Composio endpoint. The complete local check passed: 201 tests, type checking and a production build; three external integration tests remain opt-in.

- Public launch checks against the deployed application verified trial and paid access, Full access grant/revocation without redeployment, and denial of unpaid or expired-trial processing. The synthetic mailbox had no Gmail credentials and stayed paused, so these checks created no Gmail activity, Stripe customer or charge. Synthetic records were removed. The new deployment error scan was clean.
