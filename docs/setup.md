...
Set a dedicated `OPENAI_API_KEY` as a private server environment variable. Sotto calls the OpenAI Responses API directly; it does not use an AI gateway or fall back to another provider. On Vercel, save the production key as Sensitive and redeploy. Never expose it with a `NEXT_PUBLIC_` prefix or reuse another application's key. Set `OPENAI_MODEL` if desired; the default is `gpt-5-mini`. Use a model that supports the Responses API and strict JSON schema output. Verify model access and provider limits before running a real pilot. See [OpenAI API authentication](https://developers.openai.com/api/reference/overview#authentication).

Under **Settings**, describe what matters for each account in natural language. The AI receives these preferences and chooses keep, review or move from the message's meaning. Relationship protections still apply. No keyword list or confidence threshold decides whether a vendor pitch is cold.

To verify the deployed classifier independently of Gmail, call `GET /api/cron/reconcile?probe=classifier` with the `CRON_SECRET` bearer in Vercel queue mode. This classifies one synthetic vendor pitch and returns only its category/decision or a fixed diagnostic code; it never reads a mailbox. Add `&sample=campaign` to test a brand campaign. For HTTP 429, Sotto honors `Retry-After` (seconds or HTTP date), stops the current batch, defers the mailbox backlog, and retries without marking mail as classified or exhausting its retry budget. Without a usable header it waits five minutes. Ordinary processing errors still stop after eight attempts. Check [OpenAI error codes](https://developers.openai.com/api/docs/guides/error-codes) for rate or quota limits; a quota failure requires fixing billing or account limits rather than repeated retries.
...
5. Choose a dedicated push service account without project-wide roles or downloaded keys. Set its exact email in `PUBSUB_SERVICE_ACCOUNT_EMAIL` and the configured OIDC audience in `PUBSUB_AUDIENCE`. Grant the project's Pub/Sub service agent (`service-PROJECT_NUMBER@gcp-sa-pubsub.iam.gserviceaccount.com`) `roles/iam.serviceAccountOpenIdTokenCreator` **on that push service account only**. This role grants the `iam.serviceAccounts.getOpenIdToken` permission required for authenticated push without allowing OAuth access tokens or arbitrary signing. See [Google's service account authentication roles](https://docs.cloud.google.com/iam/docs/service-account-permissions).
...
Existing mail starts processing immediately after connection; no new email is required. **Accounts** and **My inbox** show processed, pending, retrying and failed counts for the complete backlog, including the start date. Processing continues with the browser closed. With write access and the new connection notice, it automatically moves eligible cold outreach within the seven-day window. Without configured Gmail push, new mail is picked up by the daily recovery cron or **Check now**, not instantly. A timestamp means Gmail changes were discovered, not that all classification jobs have finished.

Once satisfied, set `ENABLE_MAILBOX_WRITES=true`, redeploy or restart the web and worker, and activate the filter for that account from **Accounts**. The Start filtering button authorizes a seven-day cleanup plus new mail; it does not claim that a sample was reviewed. Start with unsolicited sales only. Enabling an optional category changes future decisions; it does not replay prior decisions automatically.
